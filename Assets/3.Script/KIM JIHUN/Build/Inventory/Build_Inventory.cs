...
{
    private List<Build_Item> _inventory;
    public List<Build_Item> Inventory => _inventory;

    public Build_Inventory()
    {
        _inventory = new List<Build_Item>();
    }

    public bool AddToInventory(Build_Item itemToAdd)
    {
    ...
    */
    ...
    public bool RemoveFromInventory(Build_Item itemToRemove)
    {
        /*
        같은 아이템이 있는지 확인
    ...
    /*
        public int FindSameItem(Build_Item itemToFind)
        {
            for (int i = 0; i < _inventory.Count; i++)
            {
                ...
                if (slot.itemName == itemToFind.itemName && slot.Amount < slot.ItemData.maxAmount)
                {
                    return i;
                }
            }
